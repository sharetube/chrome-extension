...
if len(args) != 2:
    print("Usage: fix-world-main-scripts.py <extension dir>")
    sys.exit(1)
...
with open(ext_dir + "/manifest.json", "r") as manifestFile:
    ...
    for i in range(len(content_scripts)):
        ...
        if content_script.get("world") == "MAIN":
            # fixme: iterate through all files
            ...
            with open(loader_file_path, "r") as loader_file:
                ...
                if not match:
                    print("Could not find content script import in loader file")
                    sys.exit(1)

                content_script_path = match.group(1)
                manifest.get("content_scripts")[i].get("js")[0] = content_script_path
