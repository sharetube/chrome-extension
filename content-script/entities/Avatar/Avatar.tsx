...
const Avatar: React.FC<AvatarProps> = ({ size = "s", url, letter, color }) => {
	...
	if (url) {
		return (
			...
			>
				<div className="absolute border-solid rounded-full top-[-1px] left-[-1px] w-full h-full border border-spec-outline pointer-events-none" />
			</div>
		);
	}
	...
	return (
		...
		>
			<p
				className={`p-0 m-auto text-white font-secondary text-center ${letterClass}`}
			>
				{letter}
			</p>
		</div>
